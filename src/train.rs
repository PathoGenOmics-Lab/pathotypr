...
//! 2. Manually training an ensemble of Decision Trees (a Random Forest).
//! 3. Bundling the trees, vectorizer, label encoder, and config into a single file.
...
// Import Decision Tree components
...
const N_TREES: u16 = 100; // Number of trees in our manual forest
...
// --- Domain structs (no changes) ---
#[derive(Debug, Clone, PartialEq, Eq)]
struct GenomeSequence(String);
impl GenomeSequence {
    pub fn new<S: Into<String>>(s: S) -> Result<Self, String> {
        let s = s.into();
        if s.trim().is_empty() { Err("Genome sequence cannot be empty".into()) } else { Ok(Self(s)) }
    }
...
impl Lineage {
    pub fn new<S: Into<String>>(s: S) -> Result<Self, String> {
        let s = s.into();
        if s.trim().is_empty() { Err("Lineage cannot be empty".into()) } else { Ok(Self(s)) }
    }
    pub fn inner(&self) -> &str { &self.0 }
}
...
fn prepare_data(texts: &[String], labels: &[String], kmer_size: usize) -> AppResult<(CountVectorizer, LabelEncoder, Vec<Vec<f64>>, Vec<usize>)> {
    ...
    if label_encoder.int_to_label.len() < 2 { return Err(AppError::NotEnoughData("Training data must contain at least two distinct classes.".to_string())); }
    Ok((vectorizer, label_encoder, x_data, y))
}

fn split_train_test(x_data: &[Vec<f64>], y: &[usize], test_ratio: f64) -> AppResult<(DenseMatrix<f64>, Vec<usize>, DenseMatrix<f64>, Vec<usize>)> {
    let n_samples = x_data.len();
    let test_size = (n_samples as f64 * test_ratio).round() as usize;
    let train_size = n_samples - test_size;
    if train_size == 0 { return Err(AppError::NotEnoughData("Not enough samples for training after splitting.".to_string())); }
    ...
    // FIX: Safely handle the creation of an empty test matrix
    let x_test = if test_data.is_empty() {
        let num_features = if train_data.is_empty() { 0 } else { train_data[0].len() };
        DenseMatrix::zeros(0, num_features)
    } else {
        DenseMatrix::from_2d_vec(&test_data)?
    };
...
pub fn run(args: Args) -> AppResult<()> {
    if !(0.0..=1.0).contains(&args.test_split) { return Err(AppError::Generic("--test-split must be between 0.0 and 1.0.".to_string())); }
    ...
    info!("▶ Starting to train an ensemble of {} decision trees -> {}", N_TREES, args.output);
    ...
    pb.set_style(ProgressStyle::default_bar().template("  Training trees: [{bar:40.cyan/blue}] {pos}/{len} ({eta})")?.progress_chars("=>-"));

    for _ in 0..N_TREES {
        let sample_indices: Vec<usize> = (0..n_samples).map(|_| rng.gen_range(0..n_samples)).collect();
        let x_sample_vec: Vec<Vec<f64>> = sample_indices.iter().map(|&i| x_train.get_row(i).iterator(0).copied().collect()).collect();
        let y_sample: Vec<usize> = sample_indices.iter().map(|&i| y_train[i]).collect();
        let x_sample = DenseMatrix::from_2d_vec(&x_sample_vec)?;
    ...
    if !x_test.is_empty() {
        let mut y_pred_votes = Vec::new();
        for i in 0..x_test.shape().0 {
            let row_vec: Vec<f64> = x_test.get_row(i).iterator(0).copied().collect();
        ...
        info!("  Test set is empty, skipping accuracy calculation.");
    }
